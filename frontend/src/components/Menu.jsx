...
export default function Menu() {
    ...
    return (
        <aside className="h-full bg-slate-800 max-md:w-auto relative z-[50]">
            <button className='focus:outline-none relative left-full top-1/2' onClick={toggleMenu}>
                {isMenuOpen ? <FaChevronLeft /> : <FaChevronRight />}
            </button>
            <ul className={`transition-all ease-in-out ${isMenuOpen ? 'w-[200px]' : 'w-0 opacity-0'} duration-[500ms] overflow-hidden flex flex-col justify-between text-stone-100`}>
