            ...
            <header className="header sticky top-0 w-full h-[10%] z-[51] bg-slate-800 flex justify-between py-2 px-6 max-lg:h-[15%]">
                ...
                <div className="flex items-center">
                    {isAuthenticated===false && (
                        <Link to='/login' className="h-12 max-lg:h-[60%]"><button className="px-5 outline-none bg-slate-100 rounded-xl text-lg h-12 max-lg:h-full">SignIn</button></Link>
                        )}
                    {isAuthenticated===true && (
                        <button className="px-5 outline-none bg-slate-100 rounded-xl text-lg h-12 max-lg:h-[60%]" onClick={handleLogout}>Logout</button>
                    )}
                </div>
            </header>
        </Headroom>
